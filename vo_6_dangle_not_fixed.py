...
class VoronoiDiagram:
    ...
    def _merge(self, left_data, right_data, step_mode):
        ...
        print(f"\n[DEBUG] --- MERGE of {len(left_points)} L-pts and {len(right_points)} R-pts ---")
        ...
        print(f"[DEBUG] Upper tangent: L[{ua_idx}]{p_ua} to R[{ub_idx}]{p_ub}")
        print(f"[DEBUG] Lower tangent: L[{la_idx}]{p_la} to R[{lb_idx}]{p_lb}")
        ...
        last_v_pt = None
        loop_count = 0
        print(f"[DEBUG] --- Begin Hyperplane Tracing ---")
        while True:
            loop_count += 1
            if loop_count > (len(left_points) + len(right_points)) * 4:
                print(f"[DEBUG] !!! Safety break in hyperplane tracing loop !!!")
                break
            
            bisector = scan_line.get_perpendicular_bisector()
            print(f"[DEBUG] Loop {loop_count}: Scan line on {scan_line.norm_start}-{scan_line.norm_end}. Bisector generated.")
            ...
            if search_origin is None:
                ...
                if clipped_pts:
                    ...
                    print(f"[DEBUG]   Searching from boundary point: {search_origin}")
                else:
                    print(f"[DEBUG]   Bisector is entirely outside canvas. Cannot start tracing.")
                    break
            ...
            print(f"[DEBUG]   Intersections: Left at {l_is_pt} (L[{l_is_idx}]), Right at {r_is_pt} (R[{r_is_idx}])")

            near_is_pt, choose_dir, near_is_idx = self._find_near_inter_is(bisector, l_is_pt, r_is_pt, l_is_idx, r_is_idx)
            side_str = 'Left' if choose_dir == 1 else 'Right' if choose_dir == 2 else 'None'
            print(f"[DEBUG]   => Chosen next vertex: {near_is_pt} from side {side_str}[{near_is_idx}]")
            ...
            if end_pt is None or is_lower:
                if is_lower: print(f"[DEBUG]   Lower tangent reached.")
                else: print(f"[DEBUG]   No valid intersection found. Ending hyperplane with a ray.")
            ...
            print(f"[DEBUG]   => Added H-segment H{len(hyperplane)-1}: {segment}")
        ...
        print(f"[DEBUG] --- Hyperplane Tracing Finished ---")
        ...
        print("[DEBUG] --- Removing Obsolete Internal Edges ---")
        for edge in l_edges:
            ...
            if p_start in new_vertices and p_end in new_vertices:
                print(f"[DEBUG]   Removing redundant L-edge: {edge}")
                continue
        ...
        for edge in r_edges:
            ...
            p_end = (round(edge.end.x, 5), round(edge.end.y, 5))
            if p_start in new_vertices and p_end in new_vertices:
                print(f"[DEBUG]   Removing redundant R-edge: {edge}")
                continue
    ...
    def _elimination(self, hyperplane, scan_trends, l_edges, r_edges):
        print("\n[DEBUG]   _elimination starts (Final Logic based on Trend)...")
        for i in range(len(hyperplane) - 1):
            turn_info = scan_trends[i]
            edge_info = scan_trends[i]

            if edge_info.direction == 0: continue
            ...
            side_name = "Left" if edge_info.direction == 1 else "Right"
            ...
            print(f"\n[DEBUG]     --- Turn Analysis at {intersection_pt} ---")
            print(f"[DEBUG]     Turn {i}: Incoming H-segment is H{i}. Causal edge is {side_name}[{edge_info.index}]: {edge_to_prune}")
            print(f"[DEBUG]     Trend is {'LEFT (CCW)' if turn_info.trend == 1 else 'RIGHT (CW)' if turn_info.trend == 2 else 'STRAIGHT'}")
            ...
            if turn_info.trend == 1: # Trend LEFT (CCW), need positive cross product
                print(f"[DEBUG]       LEFT turn requires positive cross product.")
                print(f"[DEBUG]       Cross(H_in, Keep_END) = {cross_prod_end:.2f}. Cross(H_in, Keep_START) = {cross_prod_start:.2f}")
                if cross_prod_end > cross_prod_start:
                    print(f"[DEBUG]       ACTION: Pruning START of {side_name}[{edge_info.index}]. Old Start: {original_start}, New Start: {intersection_pt}")
                    edge_to_prune.start = intersection_pt
                else:
                    print(f"[DEBUG]       ACTION: Pruning END of {side_name}[{edge_info.index}]. Old End: {original_end}, New End: {intersection_pt}")
                    edge_to_prune.end = intersection_pt
            elif turn_info.trend == 2: # Trend RIGHT (CW), need negative cross product
                print(f"[DEBUG]       RIGHT turn requires negative cross product.")
                print(f"[DEBUG]       Cross(H_in, Keep_END) = {cross_prod_end:.2f}. Cross(H_in, Keep_START) = {cross_prod_start:.2f}")
                if cross_prod_end < cross_prod_start:
                    print(f"[DEBUG]       ACTION: Pruning START of {side_name}[{edge_info.index}]. Old Start: {original_start}, New Start: {intersection_pt}")
                    edge_to_prune.start = intersection_pt
                 ...
                 print("[DEBUG]       No turn (straight). No elimination action.")
...
class Application(tk.Tk):
    ...
    def _filter_isolated_edges(self, edges, tolerance=1e-5):
        """
        [最終修正版 v2] 過濾掉孤立線段。
        此版本要求連接點必須在畫布「嚴格內部」，在邊界上的接觸不算。
        """
        # 輔助函式，定義「嚴格在畫布內」
        def is_inside_canvas(p, w, h):
            epsilon = 1e-9
            # 判斷條件從 <= 改為 <，要求點不能在邊界上
            return (epsilon < p.x < w - epsilon) and \
                   (epsilon < p.y < h - epsilon)

        # 步驟 1: 建立一個只包含唯一線段的列表 (去重)
        unique_edges = []
        seen_edges_canonical = set()
        
        for edge in edges:
            p1 = (round(edge.start.x, 5), round(edge.start.y, 5))
            p2 = (round(edge.end.x, 5), round(edge.end.y, 5))
            canonical_edge = tuple(sorted((p1, p2)))
            
            if canonical_edge not in seen_edges_canonical:
                seen_edges_canonical.add(canonical_edge)
        ...
        for i, edge_i in enumerate(unique_edges):
            ...
            for j, edge_j in enumerate(unique_edges):
                ...
                if Point.is_similar(edge_i.start, edge_j.start, tol=tolerance):
                    connection_point = edge_i.start
                elif Point.is_similar(edge_i.start, edge_j.end, tol=tolerance):
                    connection_point = edge_i.start
                elif Point.is_similar(edge_i.end, edge_j.start, tol=tolerance):
                    connection_point = edge_i.end
                elif Point.is_similar(edge_i.end, edge_j.end, tol=tolerance):
                    connection_point = edge_i.end
                
                # 連接點必須存在且「嚴格在畫布內」，才算是有效連接
                if connection_point and is_inside_canvas(connection_point, self.width, self.height):
                    is_attached = True
                    break
    ...
    def run_normal(self):
        ...
        for edge in kept_edges:
            p1 = (round(edge.start.x, 5), round(edge.start.y, 5))
            p2 = (round(edge.end.x, 5), round(edge.end.y, 5))
            kept_geometries.add(tuple(sorted((p1, p2))))
        ...
        for i, edge in enumerate(edges):
            ...
            p2 = (round(edge.end.x, 5), round(edge.end.y, 5))
            canonical_edge = tuple(sorted((p1, p2)))
            if canonical_edge not in kept_geometries:
                print(f'要刪除的線段 {i}: {edge}')
                deleted_count += 1
    ...
    def draw_all(self, points, left_edges, right_edges, hyperplane, convex_hull_pts, is_final_run=False):
        ...
        def draw_edge_with_id(edge, index, color, prefix, width=1.5):
            ...
            self.canvas.create_line(edge.start.x, convert_y(edge.start.y), 
                                    edge.end.x, convert_y(edge.end.y), 
        ...
        if is_final_run:
            ...
            for i, edge in enumerate(left_edges): 
                draw_edge_with_id(edge, i, "#a6d1e6", "L", width=1.5)
            for i, edge in enumerate(right_edges): 
                draw_edge_with_id(edge, i, "#b5e5a4", "R", width=1.5)
            for i, edge in enumerate(hyperplane): 
                draw_edge_with_id(edge, i, "blue", "H", width=2)
