...
class PointDrawerApp:
    ...
    def draw_recursive_split_lines(self, vertices, depth=0):
        if len(vertices) <= 3:
            return
        ...
        divider_x = (left[-1].x + right[0].x) / 2.0

        y1, y2 = 0, self.canvas_height
        edge_id = -1000 - depth  # 避免與其他 edge id 衝突
        self.draw_edge_on_canvas(
            Edge(edge_id, Vertex(edge_id*10+1, divider_x, y1), Vertex(edge_id*10+2, divider_x, y2)),
            "blue", 1, (5, 3), tags="divider"
        )

        self.draw_recursive_split_lines(left, depth + 1)
        self.draw_recursive_split_lines(right, depth + 1)
